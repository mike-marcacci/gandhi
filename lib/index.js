...
module.exports = function(config, app){
	...
	config.modules.forEach(function(directory){
		...
		if(fs.existsSync(directory + '/bower.json')){
			var name = require(directory + '/bower.json').name;

			// symlink into bower
			try { fs.unlinkSync(__dirname + '/app/assets/bower/' + name); } catch(e) {}
			fs.symlinkSync(directory, __dirname + '/app/assets/bower/' + name, 'dir');

			bowerJson.dependencies[name] = __dirname + '/app/assets/bower/' + name;
			bowerJson.modules.push(name);
		}
	...
	router.get('/main.js', function(req, res){
		...
		body += deps.js.map(function(file){
			return 'document.write(\'<script type="text/javascript" src="' + file.replace(new RegExp('^'+__dirname+'/app/'), '') + '"></script>\');';
		}).join('\n') + '\n';
