...
module.exports = function (config) {
   ...
   *************/
    ...
    // add resources
  var resources = {
      db: require('./api/utils/db.js')(config.db, config.pool.max,
        config.pool.min, config.pool.timeout),
  ...
  config.modules.forEach(function (directory) {
    ...
    if (fs.existsSync(directory + '/bower.json')) {
      ...
      // symlink into bower
      try {
        fs.unlinkSync(__dirname + '/app/assets/bower/' + json.name)
      } catch (e) {}
      try {
        fs.symlinkSync(directory, __dirname + '/app/assets/bower/' + json.name,
          'dir')
      } catch (e) {}

      bowerJson.dependencies[json.name] = __dirname + '/app/assets/bower/' +
        json.name
  ...
  require('bower').
    commands.
    install(undefined, undefined, { cwd: __dirname + '/../' })
  ...
  var styles = deps.css.map(function (file) {
    return 'document.write(\'<link rel="stylesheet" type="text/css" href="' +
      file.replace(new RegExp('^' + __dirname + '/app/'), '') + '" />\');'
  }).join('\n') + '\n'
  ...
  router.use('/api',
    require('./api/middleware/auth.js')(config.auth, resources))
  ...
  router.get('/embed.js', function (req, res) {
    res.header('Access-Control-Allow-Origin', '*')
    res.header('Access-Control-Allow-Headers',
      'Origin, X-Requested-With, Content-Type, Accept')
    res.send(embed + '(\'' + (req.get('host') + config.root) + '\',\'' +
      (req.params.id || 'gandhi') + '\');')
  })
