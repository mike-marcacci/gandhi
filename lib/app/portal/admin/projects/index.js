...
.config(function($stateProvider, RestangularProvider) {
	...
	$stateProvider
		...
		.state('portal.admin.projects.show.flow', {
			...
			controller: function ($scope, $state, $stateParams, Restangular, $rootScope) {
				...
				$scope.$watch('project', function(project){
					...
					_.each(project.flow, function(content, id){
						_.each(content.permissions, function(v,k){
							content.permissions[k] = true;
						})
					});
				})
			}
		...
		.state('portal.admin.projects.show.flow.stage', {
			url: '/:stage',
			template: '<div gandhi-component="{cycle: cycle, project: project, stage: stage, role: role, mode: \'default\', isAdmin: true}"></div>',
