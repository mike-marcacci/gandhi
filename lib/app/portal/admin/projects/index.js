	...
	.state('portal.admin.projects', {
		...
		controller: ['$scope', 'Project', 'Cycle', 'User', 'Export', 'Stage', function($scope, Project, Cycle, User, Export, Stage){
			...
			var columns = $scope.columns = [
				{sortable: true, active: true, primary: true, group: 'Default', title: 'Title', path: ['title'], template: '<div style="max-width: 400px;">{{value}}</div>'},
				{sortable: true, active: true, primary: true, group: 'Default', title: 'Status', path: ['status','title']},
				{sortable: false, active: true, primary: false, group: 'Default', title: 'Cycle', path: ['cycle_id'], template: '{{scope.cyclesById[value].title}}'},
				{sortable: false, active: false, primary: false, group: 'Default', title: 'Assignments', path: ['assignments'], template: '<div ng-repeat="a in value"><a title="{{scope.cyclesById[row.cycle_id].roles[a.role_id].title}}" ui-sref="portal.admin.users.show({user: a.id})">{{scope.usersById[a.id].name}}</a></div>', include: 'assignments'},
				{sortable: true, active: true, primary: false, group: 'Default', title: 'Created', path: ['created'], template: '{{row.created * 1000 | date:"yyyy.MM.dd - hh:mm a"}}'},
				{sortable: true, active: true, primary: false, group: 'Default', title: 'Updated', path: ['updated'], template: '{{row.updated * 1000 | date:"yyyy.MM.dd - hh:mm a"}}'}
			];
