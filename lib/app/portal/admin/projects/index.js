	...
	.state('portal.admin.projects', {
		...
		controller: ['$scope', 'Project', 'Cycle', 'User', 'Export', function($scope, Project, Cycle, User, Export){
			...
			var columns = $scope.columns = [
				{primary: true, sortable: true, active: true, title: 'Title', path: ['title']},
				{primary: true, sortable: true, active: true, title: 'Status', path: ['status','title']},
				{primary: false, sortable: true, active: true, title: 'Created', path: ['created'], template: '{{row.created * 1000 | date:"yyyy.MM.dd - hh:mm a"}}'},
				{primary: false, sortable: true, active: true, title: 'Updated', path: ['updated'], template: '{{row.updated * 1000 | date:"yyyy.MM.dd - hh:mm a"}}'}
			];
			...
			$scope.$watch('table.query.cycle', function(cycle_id) {
				...
				Export.query({
					...
					$scope.columns = _.union(columns, exports.map(function(e){
						return {title: e.title, path: ['values', e.id], template: e.template, sortable: true};
					}));
				});
			});
