		...
		.state('portal.admin.projects', {
			...
			controller: function($scope, Restangular){
				...
				$scope.$watch('constraints.cycle', function(cycle){
					if(!cycle)
						return delete $scope.table.query.filter['[cycle_id][eq]'];
		...
		.state('portal.admin.projects.show', {
			...
			controller: function($scope, $rootScope, Restangular, $state, $stateParams, $window, uuid){
				...
				$scope.mailInvitation = function(invitation) {
					...
					+ '?Subject=Invitation to %22'+$scope.project.title+'%22'
					+ '&Body=Hi%20'+invitation.name+'%2C'
						+ '%0A%0AYou%20are%20being%20invited%20to%20join%20the%20project%20%22'+$scope.project.title+'%22%20with%20the%20role%20of%20'+invitation.role+'.'
						+ '%0A%0APlease%20sign%20up%20for%20an%20account%20at%20'+$window.location.protocol+'%2F%2F'+$window.location.host+'%20then%20use%20the%20following%20invitation%20code%20to%20join%20the%20project%3A'
						+ '%0A%0A'+$scope.project.id+'%3A'+invitation.id
				};
