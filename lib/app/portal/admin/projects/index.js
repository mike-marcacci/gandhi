...
.config(function($stateProvider) {
	...
	.state('portal.admin.projects', {
		url: '/projects?cycle&user&search',
		abstract: true,
		template: '<div ui-view></div>',
		controller: ['$scope', '$state', '$stateParams', 'Project', 'Cycle', 'User', 'Stage', function($scope, $state, $stateParams, Project, Cycle, User, Stage){
			$scope.selected = {};
			...
			$scope.table = {
				query: {
					...
					cycle: typeof $stateParams.cycle === 'string' ? $stateParams.cycle : null,
					user: typeof $stateParams.user === 'string' ? $stateParams.user : null,
					search: typeof $stateParams.search === 'string' ? $stateParams.search : null,
				},
				pages: {},
				columns: columns
			};
			...
			$scope.$watchGroup(['table.query.cycle', 'table.query.user'], function() {
				$state.go('portal.admin.projects.list', {
					cycle: $scope.table.query.cycle || null,
					user: $scope.table.query.user || null,
					search: $scope.table.query.search || null
				})
			});
			...
			$scope.selected.cycle = $scope.table.query.cycle;
			...
			$scope.toggleColumn = function(column) {
				...
				if(column.include) {
					var includes = _.uniq($scope.columns.filter(function(c){
			...
			$scope.$watch('columns', function(c){
				if(c) $scope.table.columns = _.filter(c, {active: true});
			}, true);
			...
			$scope.$watch('projects', function(projects){
				...
				// get cycles
				var cycleIds = _.uniq(_.map(projects,'cycle_id'));
			...
			$scope.$watch('cycleIds', function(cycleIds){
				...
				Cycle.query({
					...
					$scope.listScope.cyclesById = _.keyBy(cycles, 'id');
				});
			});
			...
			$scope.$watch('assignedUserIds', function(assignedUserIds){
				...
				User.query({filter: [{op: 'in', path: '/id', value: assignedUserIds}]}).$promise.then(function(users){
					...
					$scope.listScope.usersById = _.keyBy(users, 'id');
				});
			});
			...
			function getProject(){
				...
				Project.get({
					...
					Role.query({
						...
						$scope.roles = roles;
						$scope.rolesById = _.keyBy(roles, 'id');
					});
					...
					Status.query({
						...
						$scope.statusesById = _.keyBy(statuses, 'id');
					});
					...
					Stage.query({
						...
						$scope.stages = stages;
						$scope.stagesById = _.keyBy(stages, 'id');
					});
				});
			}
			getProject();
		...
		controller: ['$scope', '$state', '$stateParams', '$window', 'ProjectAssignment', 'User', 'Role', function($scope, $state, $stateParams, $window, ProjectAssignment, User, Role) {
			...
			function getProjectAssignments(){
				ProjectAssignment.query({
					...
					$scope.assignmentsById = _.keyBy(assignments, 'id');
					$scope.backup = {};
				});
			}
			...
			function getUsers(){
				User.query({
					...
					$scope.users = users;
					$scope.usersById = _.keyBy(users, 'id');
				});
			}
			getUsers();
		...
		controller: ['$scope', '$state', '$stateParams', '$window', 'ProjectInvitation', 'User', 'Role', function($scope, $state, $stateParams, $window, ProjectInvitation, User, Role) {
			...
			function getProjectInvitations(){
				ProjectInvitation.query({
					...
					$scope.invitationsById = _.keyBy(invitations, 'id');
					$scope.backup = {};
				});
			}
		...
		controller: ['$scope', '$state', '$stateParams', '$window', 'Trigger', function($scope, $state, $stateParams, $window, Trigger) {
			...
			$scope.$watch('project', function(project){
				...
				Trigger.query({
					...
					$scope.triggersById = _.keyBy(triggers, 'id');
				});
			});
