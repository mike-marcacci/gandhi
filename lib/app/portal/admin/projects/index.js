	...
	.state('portal.admin.projects', {
		...
		controller: ['$scope', 'Project', 'Cycle', 'User', 'Stage', function($scope, Project, Cycle, User, Stage){
			...
			var columns = $scope.columns = [
				{group: 'default', title: 'Title',       flex: 4, sortable: true,  active: true,  primary: true,  path: ['title'],         template: '<a ui-sref="portal.admin.projects.show({project: value})" ng-click="$event.stopPropagation()">{{value}}</a>'},
			...
			$scope.$watchCollection('columns', function(columns){
				if(!columns) return;
				var columnsByGroup = _.groupBy(columns, 'group');
				$scope.defaultColumnGroup = columnsByGroup.default;
				if(columnsByGroup.contents) $scope.contentsColumnGroup = _.groupBy(columnsByGroup.contents, 'content');
			});
