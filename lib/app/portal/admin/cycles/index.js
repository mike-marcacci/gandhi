		...
		.state('portal.admin.cycles', {
			...
			controller: function($scope, Restangular){
				...
				function getList(query){
					Restangular.all('cycles').getList(query || $scope.query).then(function(res){
		...
		.state('portal.admin.cycles.show', {
			...
			controller: function($scope, $rootScope, Restangular, $state, $stateParams, $window){
				...
				$scope.removeUser = function(id) {
					...
					if(!confirm('Are you sure you want to remove ' + $scope.users[id].name + '?'))
						return;

					var data = {users: {}}; data.users[id] = true;
					Restangular.all('cycles').customDELETE($scope.cycle.id, null, null, data).then(function(res){
				...
				$scope.$watch('cycle', function(newValue, oldValue){
						...
						$scope.cycleUsersEdit = Object.create(newValue.users);
						...
						newValue.getList('users').then(function(res){
							$scope.users = _.indexBy(res.data, 'id');
						});

						$scope.newUser.roles = _.toArray(newValue.roles);
				}, true);
				...
				$scope.destroy = function(){
					...
					Restangular.all('cycles').customDELETE($scope.cycle.id, null, null, null).then(function(res){
