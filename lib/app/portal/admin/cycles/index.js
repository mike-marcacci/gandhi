...
.config(function($stateProvider) {
	...
	.state('portal.admin.cycles', {
		...
		controller: ['$scope', 'Cycle', function($scope, Cycle){
			$scope.table = {
				...
				pages: {},
				columns: [
					{primary: true, sortable: true, title: 'Title', path: ['title']},
					{primary: true, sortable: true, title: 'Status', path: ['status_id'], template: '<span style="text-transform: capitalize;">{{value}}</span>'},
					{primary: false, sortable: true, title: 'Open', path: ['open'], template: '{{value ? \'Open\' : \'Closed\'}}'},
					{primary: false, sortable: true, title: 'Created', path: ['created'], template: '{{value * 1000 | date:"yyyy.MM.dd - hh:mm a"}}'},
					{primary: false, sortable: true, title: 'Updated', path: ['updated'], template: '{{value * 1000 | date:"yyyy.MM.dd - hh:mm a"}}'}
				]
			};
	...
	.state('portal.admin.cycles.show', {
		...
		controller: ['$scope', '$state', '$stateParams', '$window', 'Cycle', 'Role', 'Status', 'Trigger', function($scope, $state, $stateParams, $window, Cycle, Role, Status, Trigger){
			$scope.permissions = {
				...
				'project:assignments:update': {
					id: 'project:assignments:update',
					title: 'Assign'
				},
	...
	.state('portal.admin.cycles.show.roles', {
		...
		controller: ['$scope', '$state', '$stateParams', '$window', 'Role', function($scope, $state, $stateParams, $window, Role) {
			...
			$scope.create = function() {
				...
				$scope.roleCreate.$save({
					admin: true,
					cycle: $scope.cycle.id,
					id: $scope.roleCreate.id
				}).then(function(){
					// clear the form
					$scope.roleCreate = null;
				});
	...
	.state('portal.admin.cycles.show.statuses', {
		...
		controller: ['$scope', '$state', '$stateParams', '$window', 'Status', function($scope, $state, $stateParams, $window, Status) {
			...
			$scope.create = function() {
				...
				$scope.statusCreate.$save({
					admin: true,
					cycle: $scope.cycle.id,
					id: $scope.statusCreate.id
				}).then(function(){
					// clear the form
					$scope.statusCreate = null;
				});
	...
	.state('portal.admin.cycles.show.triggers', {
		...
		controller: ['$scope', '$state', '$stateParams', '$window', 'Trigger', 'Status', 'Stage', 'Export', function($scope, $state, $stateParams, $window, Trigger, Status, Stage, Export) {
			$scope.triggers = [];
			...
			$scope.create = function() {
				...
				$scope.triggerCreate.$save({
					admin: true,
					cycle: $scope.cycle.id,
					id: $scope.triggerCreate.id
				}).then(function(){
					// clear the form
					$scope.triggerCreate = null;
				});
	...
	.state('portal.admin.cycles.show.exports', {
		...
		controller: ['$scope', '$state', '$stateParams', '$window', 'ObjectPath', 'Export', function($scope, $state, $stateParams, $window, ObjectPath, Export) {
			...
			$scope.create = function() {
				...
				$scope.exportCreate.$save({
					admin: true,
					cycle: $scope.cycle.id,
					id: $scope.exportCreate.id
				}).then(function(){
					// clear the form
					$scope.exportCreate = null;
				});
	...
	.state('portal.admin.cycles.show.stages', {
		...
		controller: ['$scope', '$state', '$stateParams', '$window', 'GandhiComponent', 'Stage', function($scope, $state, $stateParams, $window, GandhiComponent, Stage) {
			...
			$scope.create = function() {
				...
				$scope.stageCreate.$save({
					admin: true,
					cycle: $scope.cycle.id,
					id: $scope.stageCreate.id
				}).then(function(stage){
					// clear the form
					$scope.stageCreate = null;
				});
			...
			$scope.sortableOptions = {
				...
				stop: function() {
					...
					$scope.stages.forEach(function(stage, i){
						if(stage.order !== i) new Stage({order: i}).$update({
							admin: true,
							cycle: $scope.cycle.id,
							id: stage.id
						});
