...
.config(function($stateProvider) {
	...
	.state('portal.admin.cycles.create', {
		...
		controller: ['$scope', '$state', '$stateParams', '$rootScope', 'Cycle', function($scope, $state, $stateParams, $rootScope, Cycle){
			...
			$scope.cycleCreate.status_id = 'draft';


			// save
			$scope.create = function() {
				$scope.cycleCreate.$create(_.defaults({
					admin: true
				}, $scope.createParams)).then(function(cycle) {
		...
		controller: ['$scope', '$state', '$stateParams', '$window', 'CycleInvitation', 'User', 'Role', function($scope, $state, $stateParams, $window, CycleInvitation, User, Role) {
			...
			$scope.toggleCreate = function() {
				...
				if ($scope.invitationCreate) {
					$scope.invitationCreate.message = 'You have been invited to the cycle, "{{title}}". Please use the following token to accept your invitation: <a href="{{link}}">{{token}}</a>.';
				}
			};
