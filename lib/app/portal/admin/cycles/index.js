...
.config(function($stateProvider) {
		...
		controller: ['$scope', 'Cycle', function($scope, Cycle){
			$scope.table = {
				...
				columns: [
					...
					{primary: true, title: 'Status', path: ['status'], template: '<span style="text-transform: capitalize;">{{row.status}}</span>'},
	...
	.state('portal.admin.cycles.show', {
		...
		controller: ['$scope', '$state', '$stateParams', '$window', 'Cycle', 'Role', 'Status', 'Trigger', function($scope, $state, $stateParams, $window, Cycle, Role, Status, Trigger){
			...
			$scope.cycleInfoEdit = false;
			$scope.toggleInfoEdit = function(){
				if($scope.cycleInfoEdit) $scope.cycle.info = angular.copy(cycleBackup.info);
				$scope.cycleInfoEdit = !$scope.cycleInfoEdit;
			};
			...
			$scope.updateInfo = function(){
				new Cycle({info: $scope.cycle.info}).$update({id: $scope.cycle.id}).then(function(){
					$scope.cycleInfoEdit = false;
				});
			};
	...
	.state('portal.admin.cycles.show.triggers', {
		...
		controller: ['$scope', '$state', '$stateParams', '$window', 'Trigger', function($scope, $state, $stateParams, $window, Trigger) {
			...
			function getTriggers(){
				$scope.triggers = Trigger.query({cycle: $stateParams.cycle});
				$scope.backup = {};
			}
			getTriggers();
