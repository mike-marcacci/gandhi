		...
		.state('portal.admin.users', {
			...
			controller: function($scope, Restangular){
				...
				function getUsers(query){
					...
					}).all('users').getList(query || $scope.query).then(function(res){
						$scope.users = res.data;
						console.log(JSON.parse(res.headers('Pages')))
					});
				}

				$scope.$on('users', function(){ getUsers(); });
				$scope.$watch('query', function(query){ getUsers(query); }, true);
			}
		})
