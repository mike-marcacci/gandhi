		...
		.state('portal.admin.users', {
			...
			controller: function($scope, Restangular){
				...
				function getList(query){
					query = _.mapValues(query || $scope.table.query, function(v){ return JSON.stringify(v); });
					Restangular.all('users').getList(query).then(function(res){
			...
			controller: function($scope, $rootScope, Restangular, $state, $stateParams, $window, auth){
				...
				function getObject(){
					...
					Restangular.one('users', $stateParams.user).get().then(function(res){
						...
						$scope.userSource = JSON.stringify(Restangular.stripRestangular($scope.user), null, '\t');
					});
