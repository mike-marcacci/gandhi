		...
		controller: ['$scope', '$state', '$stateParams', 'Cycle', 'Project', function($scope, $state, $stateParams, Cycle, Project) {
			...
			$scope.table = {
				...
				columns: [
					...
					{sortable: true, title: 'Cycle',   path: ['cycle_id'],          template: '<a ui-sref="portal.cycles.show({cycle: value})" ng-click="$event.stopPropagation()">{{scope.cyclesById[value].title}}</a>'},
