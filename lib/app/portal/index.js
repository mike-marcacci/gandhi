...
.config(['$stateProvider', function($stateProvider) {
	...
	$stateProvider
		...
		.state('portal.dashboard', {
			...
			controller: ['$scope', '$window', '$location', 'Cycle', 'CycleAssignment', 'Project', 'ProjectAssignment', function($scope, $window, $location, Cycle, CycleAssignment, Project, ProjectAssignment){
				...
				function loadPendingReviews () {
					...
					Project.query({
						...
						$scope.pendingReviews = _(projects)
							.groupBy('cycle_id')
							.map(function(projects, cycleId) {
								return _(projects)
									.map(function(project) {
										return _(project.contents)
											.map(function(content, stageId) {
												return {
													project: project,
													cycle: $scope.cyclesById[cycleId],
													content: content,
													stage: ($scope.cyclesById[cycleId] || {stages:{}}).stages[stageId]
												};
											})
											// get only the "review" stages and contents
											.filter(function(row) {
												return row.stage && row.stage.component.name === 'review';
											})
											// get only those for which we still have write priviliges
											.filter(function(row) {
												return row.content.authorizations.write;
											})
											// get only those for which our review is not locked
											.filter(function(row) {
												return !row.content.data[$scope.currentUser.id] || !row.content.data[$scope.currentUser.id].locked;
											})
											.value();
									})
									.flatten()
						...
						// hide the whole thing if there are no pending reviews
						if (!_.keys($scope.pendingReviews).length)
							$scope.pendingReviews = null;
				...
				function getCycles(){
					Cycle.query({
						...
						$scope.cycles = cycles;
						$scope.cyclesById = _.keyBy(cycles, 'id');
						loadPendingReviews();
					});
				}
				getCycles();
