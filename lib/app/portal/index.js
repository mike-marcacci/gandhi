		...
		.state('portal', {
			...
			controller: function($scope, Restangular){
				...
				$scope.$watch('currentUser', function( currentUser, oldValue ) {
					...
					Restangular.all('cycles').getList().then(function(res){
						$scope.cycles = _.sortBy(res.data, 'title').reverse().map(function(c){
							c.isOpen = (typeof c.open === 'string') ? new Date(c.open).getTime() < Date.now() : c.open;
							if(c.isOpen) c.isOpen = (typeof c.close === 'string') ? new Date(c.close).getTime() > Date.now() : !c.close;
							return c;
						})
					});
					...
					// TEMPORARY HACK
					query.filter.cycle_id = {ne: '128f2348-99d4-40a1-b5ab-91d9019f272d'};

					Restangular.all('projects').getList(query).then(function(res){
						$scope.projects = res.data;
						$scope.projectsByCycleId = _.groupBy(res.data, 'cycle_id');
					});
				});
