		...
		controller: ['$scope', '$state', '$stateParams', 'Cycle', function($scope, $state, $stateParams, Cycle) {
			...
			$scope.table = {
				...
				columns: [
					...
					{sortable: true, title: 'Status',  path: ['status_id'], template: '<span style="text-transform: capitalize;">{{row.status_id}}</span>'},
					{sortable: true, title: 'Open',    path: ['open'],      template: '{{row.open ? \'Yes\' : \'No\'}}'},
