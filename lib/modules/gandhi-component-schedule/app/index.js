	...
	return {
		...
		controller: ['$scope', '$element', '$attrs', '$window', 'Stage', function($scope, $element, $attrs, $window, Stage) {
			...
			$scope.selectSlot = function(slot) {
				if(!$scope.content.authorizations.write)
					return;
				...
				$scope.content.data.begin = $scope.selected ? $scope.selected.begin : null;
				$scope.content.data.end = $scope.selected ? $scope.selected.end : null;
			};
	...
	return {
		...
		controller: ['$scope', '$element', '$attrs', 'Stage', function($scope, $element, $attrs, Stage) {
			...
			$scope.updateSlots = function(){
				new Stage({component:{options:{slots: _.keyBy($scope.slots, 'id') }}}).$update({
					admin: true,
