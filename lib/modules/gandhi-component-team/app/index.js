...
.directive('gandhiComponentTeam', function() {
	return {
		...
		function($scope, $element, $attrs, $state, $q, $window, $stateParams, User, Project, Content, Role, ProjectAssignment, ProjectInvitation) {
			...
			$scope.backupAssignment = {};
			$scope.toggleEdit = function(assignment) {
				if(!$scope.backupAssignment[assignment.id])
					return ($scope.backupAssignment[assignment.id] = angular.copy(assignment));

				// revert to backup
				angular.extend(assignment, $scope.backupAssignment[assignment.id]);
				delete $scope.backupAssignment[assignment.id];
			};
			...
			$scope.backupInvitation = {};
			$scope.toggleEditInvitation = function(invitation) {
				if(!$scope.backupInvitation[invitation.id])
					return ($scope.backup[invitation.id] = angular.copy(invitation));

				// revert to backup
				angular.extend(invitation, $scope.backupInvitation[invitation.id]);
				delete $scope.backupInvitation[invitation.id];
			};
