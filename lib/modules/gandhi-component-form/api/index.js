	...
	resources.components.form = {
		...
		content: {
			get: function(content, stage, project, cycle) {
				if(!content.authorizations['project:read']) return _.extend({}, content, {data: {}});
				return content;
			},
			update: function(data, content, stage, project, cycle) {
				if(!content.authorizations['project:write']) return Q.reject({code: 403, message: 'You are not authorized to write this content.'});
				return data;
			},
