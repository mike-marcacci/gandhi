		...
		controller: function($scope, $element, $attrs, $rootScope, Restangular, $state) {
			...
			$scope.$watch('context', function(context) {
				...
				$scope.$watch('data.schema', function(schema){
					$scope.schema = JSON.stringify(schema ? Restangular.stripRestangular(schema) : {type: 'object', properties: {}}, null, 2);
				});

				// process the form
				$scope.$watch('data.form', function(form){
					$scope.form = JSON.stringify(form ? Restangular.stripRestangular(form) : ['*'], null, 2);
				});
