	...
	return {
		...
		controller: ['$scope', '$element', '$attrs', '$window', 'Stage', function($scope, $element, $attrs, $window, Stage) {
			$scope.$watch('context', function(context) {
				...
				// context.stage.component.options.schema.readOnly = !context.content.authorizations.write

				// set schema-form options
				$scope.sfOptions = {
					supressPropertyTitles: true,
					readOnly: !context.content.authorizations.write
				}
			});
			...
			$scope.draft = function(){
				...
				$scope.content.$update({project: $scope.project.id, id: $scope.content.id}).then(function(content){
