...
Project.create = function(conn, data, cycle, user, listeners) {
...
Project.prototype.save = function(conn) {
	...
	.then(function(events) {
		...
		return Promise.all(
			...
			// update the events object on 
			Object.keys(events).map(function(trigger_id) {
				self.events[trigger_id] = self.events[trigger_id] || [];
				self.events[trigger_id].unshift(events[trigger_id]);

				// emit the new event
				return self.emit('trigger', conn, trigger_id, events[trigger_id]);
			})
