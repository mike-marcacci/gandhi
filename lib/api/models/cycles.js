...
// requires cycle('role')
function processPermissions(group, cycle) {

	// get simulated events object
	var events = processTriggers(cycle);

	return group.coerceTo('array').map(function(p){
		return [p.nth(0), projects.hasPermission(p.nth(1), cycle('role'), events)];
	}).coerceTo('object');
}
...
function build(user, cycle) {
	...
	.merge(function(cycle){
		return { authorizations: processPermissions(cycle('permissions'), cycle) };
	});
}
