...
function getRole(user, cycle) {
	return cycle('roles')(cycle('assignments')(user('id'))('role_id')).default(
		r.branch(
			user('admin'),
			true,
...
function build(user, cycle) {
	...
	.merge(function(cycle){
		return { role: user === true ? true : getRole(user, cycle) };
	})
	...
	.merge(function(cycle){
		...
		var role = r.branch(cycle('role').typeOf().eq('BOOL'),
			cycle('roles')(cycle('defaults')('role_id')),
			cycle('role')
		);

		// calculate permission requirements
		var constraints = cycle('permissions')('create')(role('id')).default(false);
