...
function User (conn, data) {
  ...
  if (typeof data === 'undefined')
    throw new Error(
      'All constructor arguments are required by Model constructors.')
...
User.validate = function (data) {
  return validator.validate('http://www.gandhi.io/schema/user', data,
    { useDefault: true, removeAdditional: true })
}
User.create = function (conn, data) {
  ...
  if (typeof data.password === 'string') try {
    data.password = scrypt.kdfSync(data.password, scryptParams).
      toString('base64')
  } catch (err) {
  ...
  if (typeof data.recovery_token === 'string') try {
    data.recovery_token = scrypt.kdfSync(data.recovery_token, scryptParams).
      toString('base64')
  } catch (err) {
    return Promise.reject(err)
  }

  return new User(conn, data).then(function (user) {
...
User.prototype.update = function (conn, delta) {
  ...
  if (typeof delta.password === 'string') try {
    delta.password = scrypt.kdfSync(delta.password, scryptParams).
      toString('base64')
  } catch (err) {
    return Promise.reject(err)
  }
  ...
  if (typeof delta.recovery_token === 'string') try {
    delta.recovery_token = scrypt.kdfSync(delta.recovery_token, scryptParams).
      toString('base64')
  } catch (err) {
    return Promise.reject(err)
  }
...
User.prototype.verifyPassword = function (password) {
  let self = this
  const passwordBuffer = Buffer.from(self.password, 'base64')
  return scrypt.verifyKdf(passwordBuffer, password)
}

User.prototype.verifyRecoveryToken = function (token) {
  var self = this
  const recoveryBuffer = Buffer.from(self.recovery_token, 'base64')
  return scrypt.verifyKdf(recoveryBuffer, token)
}
