				...
				return r.branch(
					...
					r.branch(
						...
						r.branch(
							...
							// export triggers
							r.branch(
								condition('name').eq('export_regex'),
								project('exports')(condition('options')('export_id').default('')).default('').match(condition('options')('regex').default('')).not().not(),

								// DEPRECATED: regex triggers
								// --------------------------
								//
...
function build(user, project, cycle) {
	...
	.merge(function(project){
		return { role: user === true ? true : getRole(user, project, cycle) };
	})
