...
// RQL (OBJECT) returns exported values
function processExports(project) {
	return project('contents').coerceTo('array').map(function(e){
		return [e.nth(0), e.nth(1)('exports')];
	}).coerceTo('object');
}
					...
					r.branch(
						...
						r.branch(
							...
							r.branch(
								...
								// eq
								r.branch(
									condition('name')('options')('op').default('eq').eq('eq'),
									project('exports')(condition('options')('content_id').default(''))(condition('options')('export_id').default('')).default(null).eq(condition('options')('value')),

									// gt
									r.branch(
										condition('name')('options')('op').eq('eq'),
										project('exports')(condition('options')('content_id').default(''))(condition('options')('export_id').default('')).default(null).gt(condition('options')('value')),

										// matches
										r.branch(
											condition('name')('options')('op').eq('matches'),
											project('exports')(condition('options')('content_id').default(''))(condition('options')('export_id').default('')).default('').do(function(value){
												return r.branch( value.typeOf().eq('STRING'), value.match(condition('options')('value').default('')).not().not(), false);
											}),

											// contains
											r.branch(
												condition('name')('options')('op').eq('contains'),
												project('exports')(condition('options')('content_id').default(''))(condition('options')('export_id').default('')).default([]).do(function(value){
													return r.branch( value.typeOf().eq('ARRAT'), value.contains(condition('options')('value').default('')).not().not(), false);
												}),
...
function removeContext(project) {
	return project.without(['role','status','authorizations']);
}
...
function processWriteHooks(project, cycle) {
	...
	return cycle.do(function(cycle){
		...
		.merge(function(project){
			return { exports: r.literal(processExports(project)) };
		})
