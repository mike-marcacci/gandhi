		...
		return group.map(function(condition){
			...
			return r.branch(
				...
				r.branch(
					...
					condition('options')('timestamp').default(0).lt(r.now().toEpochTime()),
					condition('options')('timestamp').default(0).gt(r.now().toEpochTime())
				),
...
function processTriggers(project, cycle) {
	return cycle('triggers').coerceTo('array').map(function(t){
		...
		return [ k, r.branch(
			...
			base,
			base.insertAt(0, {value: value, date: r.now().toEpochTime()})
		)];
