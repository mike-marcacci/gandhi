...
Trigger.prototype.test = function(conn, project) {
	...
	return Promise.all(self.conditions.map(function(condition) {
		return testCondition(conn, condition, project);
	}))
	.then(function(bools) {
...
function testCondition(conn, condition, project) {
	...
	if(condition.name === 'content_status')
		return project.contents.get(conn, condition.options.content_id)
		.then(function(content) {
			return content.status_id === condition.options.status_id;
		})
		.then(function(bool) {
	...
	if(condition.name === 'export')
		...
		.then(function(content) {
			...
			try {
				...
				if(condition.options.op === 'coutains')
