...
Stage.validate = function(data) {
	return validator.validate('http://www.gandhi.io/schema/project#/definitions/stage', data, {useDefault: true, removeAdditional: true});
};
Stage.create = function(conn, data, parent) {
	
	if(!parent.authorizations['project/stages:write'])
...
Stage.prototype.update = function(conn, delta) {
	...
	if(!self.parent.authorizations['project/stages:write'])
		return Promise.reject(new errors.ForbiddenError());
