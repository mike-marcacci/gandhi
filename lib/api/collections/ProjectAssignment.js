...
module.exports = function ProjectAssignment(config, resources) {
	...
	function updateAssignment(conn, projectId, assignmentId, data, user){
		...
		// get the project
		return r.table('projects').get(projectId).do(function(project){
			var delta = {assignments: {}, updated: r.now().toEpochTime()}; delta.assignments[assignmentId] = data;
	...
	function saveAssignment(conn, projectId, assignmentId, data, user){
		...
		// get the project
		return r.table('projects').get(projectId).do(function(project){
			var delta = {assignments: {}, updated: r.now().toEpochTime()}; delta.assignments[assignmentId] = r.literal(data);
	...
	function deleteAssignment(conn, projectId, assignmentId, user){
		...
		// get the project
		return r.table('projects').get(projectId).do(function(project){
			var delta = {assignments: {}, updated: r.now().toEpochTime()}; delta.assignments[assignmentId] = true;
