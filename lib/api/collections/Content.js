...
module.exports = function Content(config, resources) {
	...
	// PRIVATE
	function _getContentDependencies(conn, projectId, contentId, user){
						...
						projectModel.addContext(r.expr(user), project).do(function(project){
							...
							return r.branch(
								...
								// return the models necessary for a component
								{
									...
									content: project('contents')(contentId).default({id: contentId, status_id: 'none'})
									.merge(function(content){
										...
										return r.branch(
											...
											{ authorizations: projectModel.processPermissions(cycle('stages')(contentId)('component')('permissions'), project) }
										);
									})
								}
							);
	...
	function getContent(conn, projectId, contentId, user){
		...
		return _getContentDependencies(conn, projectId, contentId, user)
	...
	function updateContent(conn, projectId, contentId, data, user){
		...
		.then(function(content){
			...
			// get the project
			return r.table('projects').get(projectId).do(function(project){
				var delta = {contents: {}, updated: r.now().toEpochTime()}; delta.contents[contentId] = content;
				...
				return r.branch(
					...
					// get the content
					r.branch(
						project('contents').hasFields(contentId).not(),
						r.error('{"code": 404, "message": "Content not found"}'),
	...
	function saveContent(conn, projectId, contentId, data, user){
		...
		.then(function(content){
			...
			// get the project
			return r.table('projects').get(projectId).do(function(project){
				var delta = {contents: {}, updated: r.now().toEpochTime()}; delta.contents[contentId] = r.literal(content);
