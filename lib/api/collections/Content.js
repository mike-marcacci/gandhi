...
module.exports = function Content(config, resources) {
	...
	function updateContent(conn, projectId, contentId, data, user){
		...
		// validate id
		if(typeof data.id !== 'undefined' && contentId !== data.id) return Q.reject(new errors.ValidationError('The request body\'s id did not match the URL param.'));
		...
		.then(function(content){
			...
			return r.table('projects').get(projectId).do(function(project){
				...
				}; delta.contents[contentId] = project('contents')(contentId).default({id: contentId, data: {}, exports: {}, status_id: 'none'}).merge(content);
