...
module.exports = function Stage(config, resources) {
	...
	function updateStage(conn, cycleId, stageId, data, user){
		...
		// validate id
		if(typeof data.id !== 'undefined' && stageId !== data.id) return Q.reject(new errors.ValidationError('The request body\'s id did not match the URL param.'));
						...
						r.branch(
							...
							r.table('cycles').get(cycleId).update(delta, {returnChanges: true}).do(function(result){
								return r.branch(
									...
									// return new value
									result('changes').nth(0)('new_val')('stages')(stageId).merge({
										cycle_id: cycle('id')
									})
								);
							})
