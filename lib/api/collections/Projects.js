...
Projects.prototype.query = function (conn, query, user) {
    ...
    return c.then(function (assignedCycles) {
        ...
        assignedCycles = _.keyBy(assignedCycles, 'id');
        ...
        if (user !== true) {
            projects = projects.filter(function (project) {
                return project('assignments').
                    hasFields(user.id).
                    or(r.expr(Object.keys(assignedCycles)).contains(project('cycle_id')));
            });
        }
        ...
        // NOTE: this needs to be applied AFTER permissions are tested
        ...
        return r.expr({
            ...
            results.projects.forEach(function (r) {
                c[r.cycle_id] = c[r.cycle_id] || assignedCycles[r.cycle_id] ||
                    cycles.get(conn, r.cycle_id, user).catch(function (err) {
            ...
            return Promise.props(c).then(function (c) {
                ...
                return Promise.filter(results.projects.map(function (data) {
                    ...
                    return new Project(conn, data, c[data.cycle_id], user).catch(function (err) {
                ...
                const paginated = query.limit ?
                    projects.slice(query.skip || 0, (query.skip || 0) + query.limit)
                    : projects;

                paginated.total = paginated.length;
                return paginated;
            });
        });
...
Projects.prototype.get = function (conn, id, user) {
    ...
    return r.table('projects').get(id).run(conn).then(function (data) {
        if (!data) {
            return Promise.reject(new errors.NotFoundError('Project not found.'));
        }

        // get the cycle
        return cycles.get(conn, data.cycle_id, user).then(function (cycle) {
            if (!cycle) {
                return Promise.reject(new errors.NotFoundError('Cycle not found.'));
            }
