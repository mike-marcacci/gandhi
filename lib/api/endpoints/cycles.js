...
module.exports = function(config, router, resources){
	...
	// authenticate
	router.use('/api/cycles', passport.authenticate('bearer', { session: false }));



	router.post('/api/cycles', function(req, res){
	...
	router.get('/api/cycles', function(req, res){
		resources.db.acquire(function(err, conn) {
			if(err)
				return res.error(err);
	...
	router.get('/api/cycles/:cycle', function(req, res){
		resources.db.acquire(function(err, conn) {
			if(err)
				return res.error(err);
	...
	router.patch('/api/cycles/:cycle', function(req, res){
		var removeUsers = [];
