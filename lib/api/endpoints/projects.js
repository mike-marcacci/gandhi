		...
		resources.db.acquire(function(err, conn) {
			...
			r.table('projects').get(req.params.project).run(conn, function(err, project){
				...
				r.table('projects').get(req.params.project).update(req.body, {returnVals: true})('new_val').do(function(row){
					...
					if(!req.user.admin && req.body.flow && req.body.flow.stages && req.body.flow.stages.award_agreement){
						// send email
						var html = '';
						html += '<h3>'+result.left.title+'</h3>';
						html += '<div><b>BQI Seed Award #:</b> '+result.left.flow.stages.internal_decision.data.number+'</div>';
						html += '<div><b>PI Name:</b> '+result.left.flow.stages.application.data.pi.name+'</div>';
						html += '<div><b>Department:</b> '+result.left.flow.stages.application.data.pi.department+'</div>';
						html += '<div><b>Account #:</b> '+result.left.flow.stages.award_agreement.data.account+'</div>';

						resources.mail.sendMail(_.defaults({
							// to: 'mike.marcacci@gmail.com, mike@ruelculture.com',
							to: 'cmogolden@uchicago.edu, elenaz@uchicago.edu',
							subject: 'Award for BQI Seed Project #'+result.left.flow.stages.internal_decision.data.number,
							html: html,
