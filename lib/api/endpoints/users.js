...
module.exports = function(config, router, resources){
	...
	function sanitize(o){
		...
		return _.assign(_.omit(o, ['password','recovery_token']), {href: '/api/users/' + o.id});
	}
			...
			resources.db.acquire(function(err, conn) {
				...
				r.table('users').filter({email: req.body.email}).limit(1).count().run(conn, function(err, count){
					...
					r.table('users').insert(req.body, {returnVals: true}).run(conn, function(err, result){
						...
						return res.send(201, sanitize(user));
					});
				});
			});
		})(req, res);
			...
			r.table('users').filter({email: req.body.email || null}).limit(1).run(conn, function(err, cursor){
				...
				cursor.toArray(function(err, existing){
					...
					r.table('users').get(req.params.user).update(req.body, {returnVals: true}).run(conn, function(err, result){
						...
						return res.send(200, sanitize(user));
					});
				});
			});
		});
	...
	// authenticate
	router.use('/api/users', passport.authenticate('bearer', { session: false }));
	...
	router.patch('/api/users/:user', function(req, res){
	...
	// authenticate
	router.use('/api/cycles/:cycle/users', passport.authenticate('bearer', { session: false }));



	router.use('/api/cycles/:cycle/users', function(req, res, next){
	...
	// router.post('/api/cycles/:cycle/users', function(req, res){
	// 	return create(req, res);
	// });

	router.get('/api/cycles/:cycle/users', function(req, res){
		return list(req, res);
	});

	router.get('/api/cycles/:cycle/users/:project', function(req, res){
		return show(req, res);
	});

	router.patch('/api/cycles/:cycle/users/:project', function(req, res){
		return update(req, res);
	});

	router.delete('/api/cycles/:cycle/users/:project', function(req, res){
		return remove(req, res);
	});
	...
	// authenticate
	router.use('/api/projects/:project/users', passport.authenticate('bearer', { session: false }));



	router.use('/api/projects/:project/users', function(req, res, next){
	...
	// router.post('/api/projects/:project/users', function(req, res){
	// 	return create(req, res);
	// });

	router.get('/api/projects/:project/users', function(req, res){
		return list(req, res);
	});

	router.get('/api/projects/:project/users/:user', function(req, res){
		return show(req, res);
	});

	router.patch('/api/projects/:project/users/:user', function(req, res){
		return update(req, res);
	});

	router.delete('/api/projects/:project/users/:user', function(req, res){
		return remove(req, res);
	});
};
