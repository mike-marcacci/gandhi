...
module.exports = function(config, resources) {
	return {
		...
		replace: function(req, res){
			...
			req.body.password = req.body.password ? scrypt.hash(req.body.password, scrypt.params(0.1)) : r.row('password');
			...
			resources.db.acquire(function(err, conn) {
				...
				return r.table('users').filter({email: req.body.email || null}).limit(1).run(conn, function(err, cursor){
					...
					cursor.toArray(function(err, existing){
						...
						// update the user
						return r.table('users').get(req.params.user).replace(req.body, {returnVals: true}).run(conn, function(err, result){
							resources.db.release(conn);
