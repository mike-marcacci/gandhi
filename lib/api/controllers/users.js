...
module.exports = function(config, resources) {
	...
	return {
		...
		update: function(req, res, next){
			...
			return using(resources.db.disposer(), resources.redlock.disposer('users:' + req.params.user), lockEmail(req.body.email), function(conn){
				return users.get(
					conn,
					req.params.user
				)
				.then(function(user){
					var admin = req.user && req.user.admin && req.query.admin === 'true' ? true : (req.user || false);

					// can only be updated by admin or self
					if(!admin && req.user.id !== user.id)
					...
					if(typeof req.body.admin !== 'undefined')
						req.body.admin = req.user.admin ? !!req.body.admin : false;
