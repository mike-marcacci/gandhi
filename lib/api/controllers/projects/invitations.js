		...
		create: function(req, res, next) {
			...
			.then(function(invitation) {
				...
				if(invitation.email) return resources.mail({
					...
					html: 'You have been invited to the project, "' + invitation.parent.title + '". ' +
						'Please use the following token to accept your invitation: ' +
						'<a href="' + req.protocol + '://' + (req.headers.host || req.hostname) + config.root + '/#/?invitation=' + token + '">' + token + '</a>.'
				});
			})
			.catch(function(err) {
				return next(err);
