							...
							r.table('cycles').get(project('cycle_id')).do(function(cycle){
								...
								.do(function(project){
									return r.branch(
										...
										// enforce assignment-level access control
										r.branch(
											project('assignments').hasFields(req.params.assignment).not().or(projectModel.visibleTo(project('assignments')(req.params.assignment), project('role'), cycle).not()),
										...
										r.branch(
											r.or(
												...
												r.and(
													...
													r.or(
														projectModel.visibleTo(project('assignments')(req.params.assignment), project('role'), cycle).not(),
														projectModel.assignableBy(project('assignments')(req.params.assignment), project('role'), cycle).not()
													)
												),

												// check permissions against the new assignment
												r.or(
													projectModel.visibleTo(body, project('role'), cycle).not(),
													projectModel.assignableBy(body, project('role'), cycle).not()
												)
											),
