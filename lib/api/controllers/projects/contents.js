					...
					return r.branch(
						...
						.do(function(project){
							return r.branch(
								...
								{
									cycle: cycleModel.sanitize(cycleModel.build(r.table('users').get(req.user.id), cycle)),
									stage: cycle('stages')(req.params.content),
									project: projectModel.sanitize(project),
									content: project('contents')(req.params.content).default({id: req.params.content, status: 'none'})
										...
										// return the models necessary for a component
										{
											...
											contents: cycle('stages').coerceTo('array').map(function(s){
												return [
													...
													// add defaults
													.default({id: s.nth(0), status: 'none'})
			...
			put: function(req, res, next) {
				...
				return resources.db.acquire(function(err, conn){
					...
					.then(function(changes){
						return res.send(changes.changes[0].new_val);
					})
					.catch(function(err){
						try { err = JSON.parse(err.msg); } catch(e){}
						return next(err);
