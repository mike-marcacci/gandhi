...
function prepare(o){
	...
	return _.omit(_.assign(o, {href: 'api/projects/' + o.project_id + '/contents/' + o.id}), collections);
}
			...
			return r.branch(
				...
				r.table('cycles').get(project('cycle_id')).do(function(cycle){
					return r.branch(
						...
						// build the project
						projectModel.build(r.expr({id: req.user.id, admin: req.user.admin}), project)
						...
						.do(function(project){
							return r.branch(
								...
								{
									cycle: cycleModel.sanitize(cycleModel.build(r.expr({id: req.user.id, admin: req.user.admin}), cycle)),
			...
			return resources.db.acquire(function(err, conn){
				...
				r.table('projects').get(req.params.project).do(function(project){
					return r.branch(
						...
						// get the cycle
						r.table('cycles').get(project('cycle_id')).do(function(cycle){
							return projectModel.build(r.expr({id: req.user.id, admin: req.user.admin}), project)
		...
		put: function(req, res, next) {
			...
			return resources.db.acquire(function(err, conn){
				...
				.then(function(content){
					...
					).merge(r.literal(content));

					return r.table('projects')
					.get(req.params.project)

					// TODO: save changes to `events`

					.update(update, {returnChanges: true})
					('changes').nth(0)('new_val')('contents')(req.params.content)
					.run(conn);
				})
