		...
		put: function(req, res, next) {
			...
			return resources.db.acquire(function(err, conn){
				...
				.then(function(content){
					...
					return r.table('projects').get(req.params.project).do(function(project){
						return projectModel.write(project.merge(changes)).do(function(data){
							return r.branch(
								r.table('projects').get(req.params.project).replace(data)('errors').gt(0),
								r.error('{"code": 404, "message": "Stage not found"}'),
								projectModel.read(r.expr({id: req.user.id, admin: req.user.admin}), data)('contents')(req.params.content)
							);
						});
					})
