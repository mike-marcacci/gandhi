			...
			return r.branch(
				...
				r.table('cycles').get(project('cycle_id')).do(function(cycle){
					return r.branch(
						...
						// build the project
						projectModel.addContext(r.expr({id: req.user.id, admin: req.user.admin}), project)
						...
						.do(function(project){
							return r.branch(
								...
								{
									cycle: cycleModel.sanitize(cycleModel.addContext(r.expr({id: req.user.id, admin: req.user.admin}), cycle)),
			...
			return resources.db.acquire(function(err, conn){
				...
				r.table('projects').get(req.params.project).do(function(project){
					return r.branch(
						...
						// get the cycle
						r.table('cycles').get(project('cycle_id')).do(function(cycle){
							return projectModel.addContext(r.expr({id: req.user.id, admin: req.user.admin}), project)
		...
		put: function(req, res, next) {
			...
			return resources.db.acquire(function(err, conn){
				...
				.then(function(content){
					...
					return r.table('projects').get(req.params.project).do(function(project){
						return projectModel.processWriteHooks(project.merge(changes)).do(function(data){
							return r.branch(
								r.table('projects').get(req.params.project).replace(data)('errors').gt(0),
								r.error('{"code": 404, "message": "Stage not found"}'),
								projectModel.addContext(r.expr({id: req.user.id, admin: req.user.admin}), data)('contents')(req.params.content)
							);
						});
					})
