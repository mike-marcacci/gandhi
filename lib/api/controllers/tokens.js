		...
		post: function(req, res, next) {
			...
			return using(resources.db.disposer(), function(conn) {
				...
				.then(function(users){
					...
					if(users.length > 1) return Q.reject(new Error('Multiple users found with the email address "' + email + '"' ));
					...
					if(typeof req.body.password != 'undefined'){
						...
						.catch(function(err){
							return Q.reject(new errors.UnauthorizedError('Incorrect password.'));
						})
					...
					// update the user
					return users.update(conn, {recovery_token: resources.auth.hashPassword(recovery_token)}, true)
