				...
				.then(function(users){
					...
					if(typeof req.body.token !== 'undefined'){
						...
						.then(function(decoded) {
							...
							// sign the token
							return resources.auth.signToken({admin: user.admin}, { expiresIn: 24*60*60, subject: user.id });
						});
					}
					...
					if(typeof req.body.password != 'undefined'){
						...
						.then(function(verified) {
							if(!verified) return Promise.reject(new errors.UnauthorizedError('Incorrect password.'));
						})
					...
					if(typeof req.body.recovery_token !== 'undefined'){
						...
						.then(function(verified){
							if(!verified) return Promise.reject(new errors.UnauthorizedError('Invalid recovery token.'));
						})

						// verify recovery token expiration
						.then(function(){
							const recoveryBuffer = Buffer.from(req.body.recovery_token, 'base64')
							var recovery_token = JSON.parse(recoveryBuffer.toString('utf8'));
							if(!recovery_token.expiration || recovery_token.expiration < (Date.now() / 1000) )
					...
					var recovery_token = new Buffer(JSON.stringify({
							email: user.email,
							expiration: (Date.now() + 3600000) / 1000, // 1 hour from now
