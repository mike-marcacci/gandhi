...
module.exports = function(config, resources) {
	...
	function buildResponse(user, project, cycle, callback) {

		// get the role
		var role = getRole(user, project, cycle);
			...
			resources.db.acquire(function(err, conn) {
				...
				r.table('cycles').get(req.body.cycle_id).run(conn, function(err, cycle){
					...
					r.table('projects').insert(req.body, {returnChanges: true}).run(conn, function(err, result){
						...
						return buildResponse(req.user, project, cycle, function(err, project){
		...
		list: function(req, res, next) {
			...
			resources.db.acquire(function(err, conn) {
				...
				if(req.params.user || req.query.user)
					return r.table('cycles').hasFields({users: req.params.user || req.query.user}).filter(function(row){
						// restrict to role on cycle
						return req.query.role
							? row('users')(req.params.user || req.query.user)('role').eq(req.query.role)
							: true;
					})('id').run(conn, function(err, cursor){
						cursor.toArray(function(err, cycleIds){
							query = query.filter(r.expr(cycleIds).contains(r.row('cycle_id')).or(r.row('users').hasFields(req.params.user || req.query.user)));

								// restrict to role on project
							if(req.query.role)
								query = query.filter(function(row){
									return row('users').hasFields(req.params.user || req.query.user).not()
										.or(row('users')(req.params.user || req.query.user)('role').eq(req.query.role));
								});
