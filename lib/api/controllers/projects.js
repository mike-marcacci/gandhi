...
var collections = ['assignments','invitations','contents', 'events'];
			...
			list: function(req, res, next) {
				...
				if(req.params.user || req.query.user || !req.user.admin)
					query = query.filter(function(project){
						var userId = req.user.admin ? (req.params.user || req.query.user) : req.user.id;
						var cycle = r.table('cycles').get(project('cycle_id'));
						var role = _projects.getRole(userId, project, cycle);
						return _projects.hasPermission(role, cycle('visible'), project);
					});
					...
					.do(function(project){
						return r.branch(
							...
							project.merge(function(project){
								...
								// restrict to permitted users
								var visible = role
								.and(role.hasFields('id'))
								.and(_projects.hasPermission(role, cycle('visible'), project));
