    ...
    return {
        query: function (req, res, next) {
            ...
            return using(resources.db.disposer(), function (conn) {
                return projects.query(conn, query, req.admin || req.user).then(function (projects) {
                    ...
                    return Promise.props(projects, function (project) {
                        var result = _.assign({}, project);
                        return Promise.props(includes, function (field) {
                            return project[field].query(conn, {}).then(function (c) {
                                result[field] = _.keyBy(c, 'id');
                            });
                        }).then(function () {
                            return result;
                        });
        ...
        get: function (req, res, next) {
            return using(resources.db.disposer(), function (conn) {
                return projects.get(conn, req.params.project, req.admin || req.user);
        ...
        update: function (req, res, next) {
            return using(resources.db.disposer(), resources.redlock.disposer('projects:' + req.params.project, 1000),
                function (conn) {
                    return projects.get(conn, req.params.project, req.admin || req.user).then(function (project) {
        ...
        create: function (req, res, next) {
            return using(resources.db.disposer(), function (conn) {
                ...
                // get the cycle
                return cycles.get(conn, req.body.cycle_id, user).then(function (cycle) {
                    // generate a new uuid
                    req.body.id = uuid();
        ...
        delete: function (req, res, next) {
            return using(resources.db.disposer(),
                resources.redlock.disposer('projects:' + req.params.project,
                    1000),
                function (conn) {
                    return projects.get(
                        conn,
