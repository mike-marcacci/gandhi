			...
			post: function(req, res, next) {
				...
				resources.db.acquire(function(err, conn) {
					...
					r.table('cycles').get(req.body.cycle_id).do(function(cycle){
						...
						// generate uuid
						var data = r.expr(req.body).merge({
							id: r.uuid()
						});

						// set defaults for admins
						if(req.user.admin) {
							data = data.merge({
						...
						// apply write-time queries
						data = projectModel.write(data, cycle);
						...
						return r.branch(
							...
							data.do(function(data){
								...
								// apply read-time queryes
								return projectModel.read(r.expr({id: req.user.id, admin: req.user.admin}), data, cycle).do(function(project){
									...
									return r.branch(
										...
										// insert the data
										r.branch(
											r.table('projects').insert(data)('errors').gt(0),
											r.error('{"code": 500, "message": "Error writing changes to the database."}'),

											// sanitize the project
											projectModel.sanitize(project)
										)
									);
								});
							})
				...
				resources.db.acquire(function(err, conn) {
					...
					r.table('projects').get(req.params.project).do(function(project){
						return r.branch(
							...
							// apply write-time queries
							projectModel.write(project.merge(req.body)).do(function(data){

								// apply read-time queries
								return projectModel.read(r.expr({id: req.user.id, admin: req.user.admin}), data).do(function(project){
									...
									return r.branch(
										...
										// update the project
										r.branch(
											r.table('projects').get(req.params.project).update(data)('errors').gt(0),
											r.error('{"code": 500, "message": "Error writing changes to the database."}'),
