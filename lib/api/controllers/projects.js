	...
	return {
		...
		destroy: function(req, res) {
			...
			resources.db.acquire(function(err, conn) {
				...
				// destroy the entire record
				if(!Object.keys(req.body).length)
					return r.table('projects').get(req.params.project).delete({returnVals: true}).run(conn, function(err, result){
						resources.db.release(conn);
				...
				_.each(req.body, function(value, key){
					if(indexes.indexOf(key) == -1 || !value)
						return;
				...
				if(!Object.keys(obj).length)
					return res.error(400, 'No valid properties sent to be removed.');

				return r.table('projects').get(req.params.project).replace(r.row.without(obj), {returnVals: true}).run(conn, function(err, result){
					resources.db.release(conn);
					...
					return res.send(sanitize(project));
				});
