					...
					r.table('cycles').get(req.body.cycle_id).do(function(cycle){
						...
						if(req.user.admin) {
							project = project.merge({
								status: project('status').default(cycle('defaults')('status'))
							});
						}
						...
						// build the project
						projectModel.build(r.expr({id: req.user.id, admin: req.user.admin}), project, cycle)
