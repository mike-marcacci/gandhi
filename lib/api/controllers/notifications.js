...
module.exports = function(config, resources) {
	return {
		post: function(req, res, next) {
			...
			// restrict non-admin to own notifications
		...
		patch: function(req, res, next) {
			...
			if(!req.user.admin) {
				// restrict non-admin to own notifications
				if(req.body.user_id && req.body.user_id !== req.user.id) return next({code: 403, message: 'Only an admin may send a notification to another user.'});
			...
			resources.db.acquire(function(err, conn) {
				...
				var notification = r.table('notifications').get(req.params.notification);
