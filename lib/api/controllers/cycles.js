...
module.exports = function(config, resources) {
	return {
		create: function(req, res) {
			...
			// TODO: apply all event conditions

			// TODO: apply all event listeners
		...
		list: function(req, res) {
			...
			if(req.query.open && req.query.open !== 'false')
				query = query.filter(r.row('open').eq(true).or(r.row('open').lt(r.now())).and(r.row('close').eq(false).or(r.row('open').gt(r.now()))));
