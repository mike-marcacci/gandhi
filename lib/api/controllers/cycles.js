	...
	return {
		...
		destroy: function(req, res) {
			...
			resources.db.acquire(function(err, conn) {
				...
				// destroy the entire record (if there are no projects that depend on this cycle)
				...
				var indexes = ['events', 'flow', 'roles', 'statuses', 'users'];
				var obj = {};
				...
				return r.table('cycles').get(req.params.cycle).replace(r.row.without(obj), {returnVals: true}).run(conn, function(err, result){
					resources.db.release(conn);
