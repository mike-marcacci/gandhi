		...
		query: function(req, res, next) {
			...
			return using(resources.db.disposer(), function(conn) {
				...
				.then(function(cycles) {
					var includes = controller.parseIncludes(req.query.includes, Cycle);

					// nothing to include
					if(!includes) return cycles;

					// include embedded models
					return Promise.map(cycles, function(cycle) {
						var result = _.assign({}, cycle);
						return Promise.map(includes, function(field) {
							return cycle[field].query(conn, {})
							.then(function(c) {
