...

		{
			post: function(req, res, next) {
				...
				resources.db.acquire(function(err, conn) {
					...
					// build the cycle
					.do(function(cycle){
						return cycleModel.build(r.expr({id: req.user.id, admin: req.user.admin}), cycle);
					})
			...
			list: function(req, res, next) {
				...
				if(!req.user.admin)
					query = query.filter(r.row('status_id').eq('draft').not());
			...
			get: function(req, res, next) {
				...
				return resources.db.acquire(function(err, conn) {
					...
					query = r.branch(
						query.eq(null),
						r.error('{"code": 404}'),
						cycleModel.build(r.expr({id: req.user.id, admin: req.user.admin}), query)
					);
					...
					.then(function(cycle){
						...
						if(!req.user.admin && cycle.status_id === 'draft')
							return next(404);
